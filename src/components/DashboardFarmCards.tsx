...
function DashboardFarmRow({
  ...
  return (
    <div className={`relative overflow-hidden ${cornerClass} ${isLast ? "" : "border-b border-stone-100"}`}>
      <div
        className={`absolute inset-y-0 right-0 flex w-[100px] items-center justify-center bg-stone-600 ${cornerClass}`}
...
export function DashboardFarmCards({ farms }: { farms: FarmCardSummary[] }) {
  if (farms.length === 0) {
    return (
      <Card className="mt-3">
        <p className="text-sm text-stone-500">No active farms</p>
      </Card>
    );
  }

  return (
    <Card className="mt-3 !p-0 overflow-hidden rounded-xl">
