...
import { ServiceReportForm } from "@/components/serviceForms/ServiceReportForm";
...
export default async function ServiceReportPage({
  ...
  let existing = sp.formId
    ...
    : sp.visitId
      ...
      : null;
  if (existing && existing.formKind !== "service_report") existing = null;

  const initialPayload =
    existing?.payload && typeof existing.payload === "object"
      ? (existing.payload as ServiceReportFormData)
      : null;
    ...
    <div>
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <h1 className={cn(PAGE_TITLE_CLASS, "min-w-0 truncate text-right")}>
          {existing ? "Edit Service Report" : "Service Report"}
        </h1>
      </div>
