...
import placementMap from "./maps/placement-fields.json";
import prebroodMap from "./maps/prebrood-fields.json";
import serviceReportMap from "./maps/service-report-fields.json";
import type {
  AnyServiceForm,
  PlacementForm,
  PrebroodForm,
...
type TemplateName = "service-report" | "placement" | "prebrood";

async function loadTemplateBytes(name: TemplateName): Promise<Uint8Array> {
...
async function loadTemplate(name: TemplateName) {
  return PDFDocument.load(await loadTemplateBytes(name));
}
...
async function buildServiceReportPdf(form: ServiceReportForm) {
  const template = "service-report" as const;
...
async function appendTemplatePage(doc: PDFDocument, template: TemplateName) {
...
async function buildPlacementPdf(form: PlacementForm) {
  const template = "placement" as const;
...
async function buildPrebroodPdf(form: PrebroodForm) {
  const template = "prebrood" as const;
