...
import serviceReportMap from "./templates/service-report-fields.json";
import placementMap from "./templates/placement-fields.json";
import prebroodMap from "./templates/prebrood-fields.json";
import type {
  AnyServiceForm,
  PlacementForm,
  PrebroodForm,
...
async function loadTemplate(templatePath: string) {
  const url =
    typeof window !== "undefined"
      ? templatePath
      : `${process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000"}${templatePath}`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Failed to load PDF template: ${templatePath}`);
...
async function buildServiceReportPdf(form: ServiceReportForm) {
  const templatePath = "/service-forms/service-report.pdf";
  const map = serviceReportMap as FieldMap;
  ...
  for (let p = 1; p < pages.length; p++) {
    const templateDoc = await loadTemplate(templatePath);
  ...
  return writePdf(doc, pdfFileName("Service-Report", form.farmName, form.date));
}

async function buildPlacementPdf(form: PlacementForm) {
  const map = placementMap as FieldMap;
  const doc = await loadTemplate("/service-forms/placement.pdf");
  const font = await doc.embedFont(StandardFonts.Helvetica);
  buildPlacementFields({ page: doc.getPages()[0]!, font, map }, form);
  return writePdf(doc, pdfFileName("Placement", form.farmName, form.date));
}

async function buildPrebroodPdf(form: PrebroodForm) {
  const map = prebroodMap as FieldMap;
  const doc = await loadTemplate("/service-forms/prebrood.pdf");
  const font = await doc.embedFont(StandardFonts.Helvetica);
  buildPrebroodFields({ page: doc.getPages()[0]!, font, map }, form);
  return writePdf(doc, pdfFileName("Prebrood", form.farmName, form.date));
}

/** Friendly download name (e.g. Service-Report-NORTH-RIDGE-2026-07-29.pdf). */
...
async function writePdf(doc: PDFDocument, filename: string): Promise<BuiltServicePdf> {
  const bytes = await doc.save({ updateFieldAppearances: false });
  return { bytes, filename };
}

/** Trigger a browser download from filled PDF bytes. */
export function downloadServiceFormPdf(built: BuiltServicePdf) {
  const copy = new Uint8Array(built.bytes);
