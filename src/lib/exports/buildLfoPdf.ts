...
import type { LfoSharePayload } from "@/lib/lfo/share-payload";
...
/** Black-and-white label/value LFO share, matching the saved farm PDF. */
export async function buildLfoPdfBytes(payload: LfoSharePayload): Promise<Uint8Array> {
  ...
  for (const section of payload.sections) {
    ...
    for (const row of section.rows) {
      ...
      const valueLines = row.value ? wrapText(row.value, bold, 10, valueW) : [""];
