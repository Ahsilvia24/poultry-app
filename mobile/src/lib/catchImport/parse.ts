...
export function parseCatchSheetRows(sheet: string[][]): CatchRow[] {
  ...
  for (const raw of sheet.slice(headerRowIdx + 1)) {
    ...
    rows.push({
      catchDate,
      farmCode: iCode >= 0 ? String(raw[iCode] ?? "").trim().toUpperCase() : "",
      farmName,
      flockId: iFlock >= 0 ? String(raw[iFlock] ?? "").trim().toUpperCase() : "",
      houseNo,
      headCount: iHead >= 0 ? parsePositiveInt(String(raw[iHead] ?? "")) : null,
    });
  }
  return rows;
}
